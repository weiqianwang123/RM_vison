...
bool ArmorDetector::setRoi(){ 
    ...
    //     int rows = (tl_y +int(3*hight))<armor_param.IMAGE_COLS?int(3*hight):(armor_param.IMAGE_COLS-tl_y);
...
bool ArmorDetector::setRoi(int roi_mul){ 
    if(num>armor_param.num_max||is_findBox==false){
        ...
        tl_x = tl_x>0?tl_x:0;
        tl_y = tl_y>0?tl_y:0;
        int cols = (tl_x+int(roi_mul*width))<armor_param.IMAGE_ROWS?int(roi_mul*width):(armor_param.IMAGE_ROWS-tl_x);
        int rows = (tl_y +int(roi_mul*hight))<armor_param.IMAGE_COLS?int(roi_mul*hight):(armor_param.IMAGE_COLS-tl_y);
        roi = src(Rect(tl_x,tl_y,cols,rows));
        offset = Point(tl_x,tl_y);
    }
    return true;
 ...
 */
    ...
    {
        loadParam(armor_param);
        enemy_color = armor_param.color;
        int num = 1;
        is_findBox=false;
    };
...
bool ArmorDetector::filterLightBlob(const cv::RotatedRect &rect){
    ...
    double area_rate = rect.size.height*rect.size.width/(armor_param.IMAGE_COLS*armor_param.IMAGE_ROWS);
    if(area_rate<armor_param.light_min_area_ratio||area_rate>armor_param.light_max_area_ratio){
    ...
    if(angle_>armor_param.lights_angle_max_dif){
        // cout<<"false3"<<endl;
        return false;
    }
    return true;
...
bool ArmorDetector::findLightBlobs(LightBlobs &light_blobs){
	...
	//进行形态学闭运算操作  
	morphologyEx(binary_Mat, binary_Mat, MORPH_CLOSE, elem_COLSE);
    ...
    for(auto contour:contours_light){
        if(contour.size()<armor_param.minCounterSize ){
            continue;
        }
...
void ArmorDetector::filterLightBlob_withReflection(const LightBlob &Lb1,const LightBlob &Lb2,
    ...
    //     return;
    // }
    if((abs(Lb1.rect.center.y - Lb2.rect.center.y)/abs(Lb1.rect.center.x - Lb2.rect.center.x)>armor_param.lights_xBias_yBias_ratio_min &&
    abs(Lb1.rect.center.y - Lb2.rect.center.y)/abs(Lb1.length)<10)){
        if(Lb1.rect.center.y <Lb2.rect.center.y){
            result2 = false;
        }
        else{
            result1 = false;
...
bool  ArmorDetector::isCoupleLight(const LightBlob &light_blob_i, const LightBlob &light_blob_j, double & Dist_height_rate,int &id)
{
    ...
    {
        // cout<<"lihght length rate:"<<armor_param.lights_lenght_max_ratio<<","<<light_blob_i.length / light_blob_j.length<<endl;
        return false;
    }

    //判断两个灯条的中心距及高度差
    double side_length, max_length;
    cv::Point2f sub_centers = light_blob_j.rect.center - light_blob_i.rect.center;
    // cout<<light_blob_j.rect.center<<"--"<<light_blob_i.rect.center<<endl;
    side_length = sqrt(sub_centers.ddot(sub_centers));  //中心距

    max_length = fmax(light_blob_i.length, light_blob_j.length); //最大长度
    double min_length = fmin(light_blob_i.length, light_blob_j.length);
    Dist_height_rate = side_length / max_length;
    if (Dist_height_rate > armor_param.lights_center_max_ratio ||
        Dist_height_rate < armor_param.lights_center_min_ratio) 
        //中心的高度差和长度的比值
    {
    ...
    if (angle>armor_param.cp_lights_angle_diff)
    {
    ...
    if(angle_mean<6){
        if(armor_param.lights_height_max_dif_4 < suby_len_rate ){
        ...
        if(armor_param.lights_height_max_dif_10 < suby_len_rate ){
        ...
        if(armor_param.lights_height_max_dif_15 < suby_len_rate ){
    ...
    // 利用分类器，进行装甲板判断
    cv::Point2f centers = (light_blob_j.rect.center + light_blob_i.rect.center)/2.0;
    ...
    int width_roiClassify = x_roiClassify+2*side_length>armor_param.IMAGE_ROWS?int(armor_param.IMAGE_ROWS-x_roiClassify):int(2*side_length);
    int heigth_roiClassify = y_roiClassify+3.5*max_length>armor_param.IMAGE_COLS?int(armor_param.IMAGE_COLS-y_roiClassify):int(3.5*max_length);
    Mat roi_classifier = src(Rect(x_roiClassify,y_roiClassify,width_roiClassify,heigth_roiClassify));
    // imshow("the classifier pic",roi_classifier);waitKey(1);
    
    //!!!!!
    id = classifier.inference(roi_classifier);
    // if(id==8||id==9||id==10){
    //     return false;
...
void ArmorDetector::calculateScore(ArmorBox & armor){
    Point2f img_center = Point2f(armor_param.IMAGE_ROWS/2.0,armor_param.IMAGE_COLS/2.0);
    Point2f sub_center = armor.rect.center - img_center;
    double area_weigth=0.5,center_weigth=0.2,id_weigth=0;
    // cout<<"distance of centers:"<<sqrt(sub_center.ddot(sub_center))<<endl;
    double lights_height_diff = abs(abs(armor.armor_points[0].y-armor.armor_points[2].y) - abs(armor.armor_points[1].y-armor.armor_points[3].y)) / abs(armor.armor_points[1].y-armor.armor_points[3].y);
    double area_rate = sqrt( armor.rect.size.width*armor.rect.size.height);
    double center_distance = sqrt(sub_center.ddot(sub_center));
    double id_s = 1;
    double lights_len_sum = armor.light_Blobs[0].length + armor.light_Blobs[1].length;
...
bool ArmorDetector::matchArmorBoxes(LightBlobs &light_blobs, ArmorBoxes &armor_boxes)
{
    ...
    if(light_blobs.size()==2){
        //判断灯条是否匹配
            ...
            armor_boxes.emplace_back(light_blobs[0],light_blobs[1],(Dist_height_rate>armor_param.lights_wihe_rate_boxSzie),id);
            }
        ...
        for(int i=0; i < light_blobs.size() - 1; i++){
            ...
            armor_boxes.emplace_back(light_blobs[left],light_blobs[right],(Dist_height_rate>armor_param.lights_wihe_rate_boxSzie),id);
            }
        }
    }
...
bool ArmorDetector::findArmorBoxes(){
    this->_armor_boxes.clear();
    this->_light_blobs.clear();
    if(!findLightBlobs(this->_light_blobs)){
        is_findBox = false;
        return false;
    }  
    if(!matchArmorBoxes(this->_light_blobs,this->_armor_boxes)){
        is_findBox = false;
        return false;
        }
    is_findBox = true;
...
bool ArmorDetector::setTargetBox(){
    if(is_findBox){
        num_noBox =0;
        target_box.is_empty=false;
        target_box = _armor_boxes[0];
        calculateScore(target_box);
        // double score=0,score_temp=calculateScore(_armor_boxes[0]);
        if(_armor_boxes.size()>1){
            for(int i=1;i<_armor_boxes.size();i++){
            calculateScore(_armor_boxes[i]);
            if(_armor_boxes[i].score>target_box.score){
                target_box = _armor_boxes[i];
            }
        }
        }   
    }
        ...
        num_noBox++;
        if(num_noBox>armor_param.num_noBox_max){
            target_box.is_empty=true;
        }
    }
    return true;
}
